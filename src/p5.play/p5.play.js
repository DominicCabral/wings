...
function load(p5) {
  p5.prototype.registerMethod("init", function p5PlayInit() {
    ...
    class Sprite {
      ...
      addCollider(offsetX, offsetY, w, h) {
        ...
        if (shape == "box") {
          ...
          if (
            Math.abs(Math.abs(vecs[0].x) - Math.abs(vecs[vecs.length - 1].x)) <=
              pl.Settings.linearSlop &&
            Math.abs(Math.abs(vecs[0].y) - Math.abs(vecs[vecs.length - 1].y)) <=
              pl.Settings.linearSlop
          ) {
            shape = "polygon";
            this.originMode = "center";
          ...
          if (this.originMode == "start") {
            ...
            for (let i = 0; i < vecs.length; i++) {
              let vec = vecs[i];
              vecs[i] = scaleTo(
                { x: vec.x + offsetX - centerX, y: vec.y + offsetY - centerY },
                this.tileSize
              );
            }
          }
      ...
      addJoint(spriteB, type, props) {
        ...
        props = Object.assign(props, {
          ...
          maxLength:
            props.maxLength != undefined ? scaleXTo(props.maxLength) : null,
          ...
          groundAnchorA: props.groundAnchorA
            ? scaleXTo(props.groundAnchorA)
            : new pl.Vec2(0, 0),
          groundAnchorB: props.groundAnchorB
            ? scaleXTo(props.groundAnchorB)
            : new pl.Vec2(0, 0),
          upperTranslation: props.upperTranslation
            ? scaleXTo(props.upperTranslation)
            : 1,
          lowerTranslation: props.lowerTranslation
            ? scaleXTo(props.lowerTranslation)
            : 1,
          linearOffset: props.linearOffset
            ? scaleTo(props.linearOffset)
            : new pl.Vec2(0, 0),
        });
        if (props.anchorA) {
      ...
      get immovable() {
        console.warn(
          "sprite.immovable is deprecated, use sprite.static instead"
        );
      ...
      _display() {
        let x =
          this.p.width * 0.5 - this.p.world.origin.x + this.x * this.tileSize;
        let y =
          this.p.height * 0.5 - this.p.world.origin.y + this.y * this.tileSize;
      ...
      applyForce(forceVector, forceOrigin) {
        ...
        if (forceOrigin) {
          ...
          this.body.applyForce(
            forceVector.mul(this.body.m_mass),
            forceOrigin,
            false
          );
        } else {
      ...
      moveTo(x, y, speed) {
        if (typeof x == "undefined") {
          console.error(
            "sprite.move ERROR: movement direction or destination not defined"
          );
          return;
        }
        if (typeof x != "number") {
      ...
      attractionPoint(magnitude, x, y) {
        console.warn(
          "sprite.attractionPoint is deprecated, use sprite.moveTowards instead"
        );
      ...
      rotate(angle, speed) {
        ...
        return (async () => {
          if (frames > 1) {
            ...
            while (
              Math.abs(this.rotationSpeed) < Math.abs(ang - this.rotation)
            ) {
              await p5.prototype.delay();
            }
      ...
      _changeAni(label) {
        ...
        if (
          ...
          (this.autoResetAnimations !== false &&
            this.p.world.autoResetAnimations)
        ) {
      ...
      getAnimationLabel() {
        console.warn(
          "sprite.getAnimationLabel is deprecated. Use sprite.animation.name instead."
        );
    ...
    class Group extends Array {
      ...
      contains(sprite) {
        console.warn(
          "Deprecated: use group.includes(sprite) instead of group.contains(sprite)"
        );
      ...
      update() {
        throw new Error(
          "Use the updateSprites function instead to control whether sprites are updated or not."
        );
      }
    }
    ...
    class Camera {
      ...
      on() {
        if (!this.active) {
          ...
          this.p.translate(
            -this.x + this.p.world.hw / this.zoom,
            -this.y + this.p.world.hh / this.zoom
          );
          this.active = true;
        }
      }
...
canvas {
        ...
        if (m.base) msg = m.base + ln;
          ...
          msg =
            errorMessages.generic[
              Math.floor(Math.random() * errorMessages.generic.length)
            ] + ln;
  ...
  p5.prototype.registerMethod("pre", function () {
    ...
    this.mouse.x =
      (this.mouseX - this.world.hw) / this.camera.zoom + this.camera.x;
    this.mouse.y =
      (this.mouseY - this.world.hh) / this.camera.zoom + this.camera.y;
  ...
  p5.prototype.registerMethod("post", function p5playPostDraw() {
    ...
    if (this.p5play.mouseTracking) {
      ...
      for (let s of this.p5play.mouseSprites) {
        if (
          (!this.p5play.mouseSprite?.mouse.draggable || s != ms) &&
          !sprites.includes(s)
        ) {
          s.mouse.hover = -1;
          s.mouse.left = 0;
